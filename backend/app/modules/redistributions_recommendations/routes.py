...
@recommendation_router.get("/", response_model=List[RedisRecomResponse])
...
@recommendation_router.get("/{recommendation_id}", response_model=RedisRecomResponse)
async def get_one_recommendation(
    recommendation_id: UUID, db: AsyncSession = Depends(get_async_session)
):
...
@recommendation_router.patch("/{recommendation_id}", response_model=RedisRecomResponse)
async def update_recommendation(
    ...
    db_item = await RecommendationService.get_one_recommendation(db, recommendation_id)
    return await RecommendationService.update_recommendation(db, db_item, recommendation)


@recommendation_router.delete("/{recommendation_id}", status_code=status.HTTP_204_NO_CONTENT)
