  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
              {steps.map((step) => {
                ...
                return (
                  <div
                    key={step.id}
                    className="flex items-center gap-3 p-2 rounded-md hover:bg-[var(--muted)]/50 transition-colors"
                  >
                    {isCompleted ? (
                      <CheckCircle2 className="w-5 h-5 text-[#701D0B] shrink-0" />
                    ) : (
                      <div className="w-5 h-5 rounded-full border-2 border-[var(--text)]/35 flex items-center justify-center font-mono text-xs text-[var(--text)] shrink-0">
                        {step.id}
                      </div>
                    )}
                    <span
                      className={`text-sm ${isCompleted ? "text-[var(--text)] line-through" : "text-[var(--text-h)] font-medium"}`}
                    >
                      {step.title}
                    </span>
                  </div>
        ...
        <div className="bg-[var(--card)] rounded-[var(--radius)] border border-[var(--border)] p-6 shadow-[var(--shadow)] flex flex-col justify-between relative overflow-hidden">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-[var(--muted)] rounded-full opacity-40 pointer-events-none" />
          ...
          <button
            className="mt-6 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-sm font-medium border border-[var(--border)] hover:bg-[var(--muted)] text-[var(--text-h)] transition-all group"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-lg font-semibold text-[var(--text-h)] tracking-tight">
            Quick Actions
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius)] p-5 shadow-[var(--shadow)] flex flex-col justify-between items-start hover:border-[#701D0B]/40 transition-colors group">
              <div className="w-10 h-10 rounded-lg bg-[#701D0B]/10 text-[#701D0B] flex items-center justify-center mb-4">
                <FolderPlus className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-base font-semibold text-[var(--text-h)] mb-1">
                  Create a project
                </h4>
                <p className="text-sm text-[var(--text)] mb-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius)] p-5 shadow-[var(--shadow)]">
            ...
            <nav className="space-y-3.5">
              {[
                ...
                <a
                  key={idx}
                  href="#"
                  className="flex items-center justify-between text-sm font-medium transition-colors hover:text-[#701D0B] group text-[var(--text-h)]"
                >
                  <span className="flex items-center gap-2.5">
                    <link.icon className="w-4 h-4 text-[var(--text)] group-hover:text-[#701D0B] transition-colors" />
                    {link.label}
                  </span>
                  {link.external ? (
                    <ExternalLink className="w-3.5 h-3.5 text-[var(--text)] opacity-60" />
                  ) : (
                    <ChevronRight className="w-4 h-4 text-[var(--text)] opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                  )}
                </a>
              ))}
            </nav>
