...
// backend sends "in-progress", "in_progress", "In Progress", etc.
...
export function MyTaskDialog({ open, onOpenChange, task }: MyTaskDialogProps) {
        ...
        {task ? (
          <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar px-6 pb-6">
            <div className="flex items-start justify-between gap-4">
              {/* Title */}
              <h2 className="mb-4 flex-1 text-2xl font-semibold text-foreground mt-2">
                {task?.name}
              </h2>
            ...
            <div className="divide-y divide-border/60">
              ...
              <MetaRow icon={<CircleCheck className="size-4" />} label="Status">
                ...
                >
                  ...
                  />
                  {formatStatusLabel(task?.status ?? "not_started")}
                </Badge>
              </MetaRow>
            ...
            {/* Submission Form (for in-progress tasks only) */}
            {isInProgress && submissionOpen && (
